    ...
    for (uint64_t i = 0  ; i < l-2ULL; i++) {
        ...
        if (((b>>i) & 0b111) == 0b011){
            ...
            if (i>0 && ((b>>(i-1))&0b111) == 0b000){
                uint64_t res = dfs(b>>(i+1ULL), l-(i+1ULL),ones- ones_before_cut) +
                              dfs(b&((1ULL<<i)-1ULL),i,ones_before_cut-1ULL);
                if (res < m){
                    m = res;
                    if (m <= 1) {
                        return m;
        ...
        if (((b>>i) & 0b111) == 0b110){
            ...
            if (i<l-2ULL && ((b>>(i+1ULL))&0b111) == 0b000){
                uint64_t res = dfs(b>>(i+3ULL), l-(i+3ULL),ones- ones_before_cut-2ULL) +
                              dfs(b&((1ULL<<(i+2ULL))-1ULL),i+2ULL,ones_before_cut+1ULL);
                if (res < m){
                    m = res;
                    if (m <= 1) {
                        return m;
...
uint64_t split(uint64_t b, uint64_t l, uint64_t ones){
    ...
    for (uint64_t i = 0; i<l-3; i++){
        ...
        if  (((b>>i) & 0b1111) == 0b1000){
            if (end == 0){
                uint64_t m =split(b>>(i+1), l-(i+1), ones);
                return m;
            } else {
                uint64_t m = dfs(b&((1ULL<<(end+1ULL))-1ULL), end, ones_before_cut) + 
